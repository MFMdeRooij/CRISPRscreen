# Use the CRISPRScreenAnalysis.R output files, put this script in the same folder, add BrunelloHGNC.csv to this folder, adjust the settings, and run the script in Rstudio.
...
#BiocManager::install(c("clusterProfiler","msigdbr","enrichplot","org.Hs.eg.db","AnnotationDbi","ReactomePA","HGNChelper","pathview", "ggplot2", "STRINGdb","ggtree"))
...
# Up to 1100 gene symbols are outdated in the Brunello annotation
BrunelloHGNC<- read.csv("BrunelloHGNC.csv")
...
for (com in comparisons) {
  ...
  # Adjust gene symbols to recent hugo symbols from file
  data$GeneSymbolHGNC<- data$GeneSymbol
  data$GeneSymbolHGNC[data$GeneSymbolHGNC %in% BrunelloHGNC$wrongGeneSymbol] <- 
  ...
  # Remove duplicate Hugo symbols, take the one with highest median fold change if the original is not there
  ...
  for (g in unique(dataDupl$Hugo)){
    ...
    idmax<- temp$GeneSymbol[temp$MedianFoldChange==max(abs(temp$MedianFoldChange))][1]
    if (sum(temp$GeneSymbol==g)==1){
      data<- rbind(data, dataDupl[dataDupl$GeneSymbol==g,])
      } else{
      data<- rbind(data, dataDupl[dataDupl$GeneSymbol==idmax,])
    }
  }
  ...
  data$fdr<- apply(data[,c("fdrDepleted","fdrEnriched")], 1, FUN=min)
  
  hitsDown<- data$Hugo[data$fdrDepleted < 0.1]
  hitsUp<- data$Hugo[data$fdrEnriched < 0.1]
  hits<- c(hitsDown,hitsUp)
  hitsDown50<- data$Hugo[order(data$MedianFoldChange)][1:50]
  hitsUp50<- data$Hugo[order(data$MedianFoldChange, decreasing = T)][1:50]
  ...
  pdf(paste0("GSEA_STRING/GSEA_",GSEAplots@setType,"_", com,".pdf"), 10,7)
