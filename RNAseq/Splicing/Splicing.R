...
# also adjust the most upstream and downstream loci in SplicingHg38.sh
...
# script (./SplicingHg38.sh on the command line) 
# The first time, make the bash script excutable (chmod 755 SplicingHg38.sh)
# You can also run the bash script here by uncommenting and run the next line:
#system("bash SplicingHg38.sh")
...
for (i in 1:nrow(dfID)){
  ...
  setwd(paste0(wd,ID))
  GOI<-data.frame(seqnames=1,pos=1,count=1)
  splice_GOI<-data.frame(seqnames=1,pos=1,count=1)
  ...
  pdf(paste0("../", ID,".pdf"),10,5)
    ...
    plot(df$pos, df$count, type="l" , cex=0.1, xlim=xlim, ylim=c(0, ylim), xlab="", xaxt="n", ylab="depth", col="white" , main=main)
    polygon(c(min(df$pos)-1,df$pos, max(df$pos)+1), c(0,df$count,0), col="lightblue", border=F)
    polygon(c(min(dfs$pos)-1,dfs$pos, max(dfs$pos)+1), c(0,dfs$count,0), col="black", border=F)
    ...
    plot(df$pos, df$count, type="l" , cex=0.1, xlim=xlim, ylim=c(ylim/-6, ylim), xlab=chr, ylab="depth", col="white" , main=main)
    polygon(c(min(df$pos)-1,df$pos, max(df$pos)+1), c(0,df$count,0), col="lightblue", border=F)
    polygon(c(min(dfs$pos)-1,dfs$pos, max(dfs$pos)+1), c(0,dfs$count,0), col="black", border=F)
    abline(h=0)
