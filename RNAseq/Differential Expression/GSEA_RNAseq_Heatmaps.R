...
if (PadjCutoff < 1){
  GSEA<- read_excel("GSEA_STRING/GSEA_ClusterProfiler_DESeq2_RNAseq.xlsx", sheet=PathSet)
}
...
# ColIDs
colLabels<- do.call(paste, c(RNAseqDesign[-1], sep = "_"))
...
anno_data <- mget(paste0("Sample_", target_cols))
names(anno_data) <- target_cols

col_list <- mget(paste0("col_funGroupSample_", target_cols))
names(col_list) <- target_cols
