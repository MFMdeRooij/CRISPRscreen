...
- In the TestData folder is a testfile available, which contains the first 750k reads of the adhesion CRISPR screen. Barcode 1-3 corresponds to the preadhesion samples, BC4:6 to the anti-IgM-induced adhesion samples, and BC7:9 to the PMA-induced adhesion samples. The easiest way to analyze this, is to run the FastqToCountTable.py in Spyder, and subsequently the R script CRISPRScreenAnalysis.R in R studio. You should get the same data as in the TestData/Output folder.
