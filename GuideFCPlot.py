...
for df,dfrra in [(dfM,dfMrra),(dfP,dfPrra),(dfC,dfCrra)]:  
    ...
    if i==2:
        sns.distplot(df['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='black', label='All guides')
        sns.distplot(df[df['Type']=='n']['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='orange', label='Non-expressed genes') 
        sns.distplot(df[df['padj']<0.1]['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='darkviolet', label='Significant (FDR < 0.1)')
    else:
        sns.distplot(df['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='black', label=None)
        sns.distplot(df[df['Type']=='n']['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='orange', label=None) 
        sns.distplot(df[df['padj']<0.1]['l2fc'], hist=False, kde_kws = {'shade': False, 'linewidth': 2}, color='darkviolet', label=None)      
    plt.xlim(xmin,xmax)
    if i==0:
    ...
    plt.xlim(xmin,xmax)   
    plt.yticks(np.arange(1,len(genes)), genes)
...
plt.savefig('H:/BioWin/GuideFCPlot.pdf', bbox_inches='tight', transparent=True)
