...
pdf(paste0("CRISPR_SL_",cellID,"_R.pdf"),7,7)
  par(mar=c(1,1,0,0))
  par(fig=c(0.1,0.75,0.1,0.75))
  plot(Combi$Nmfc.x, Combi$Nmfc.y, xlab=xlab, ylab=ylab, 
  ...
  if (GeneSymbol == 1){
    text(Combi$Nmfc.x, Combi$Nmfc.y, labels=Combi$GeneSymbol, cex=1, col="gray", adj = c(-0.2,0.5), srt=22.5)
  }
  ...
  if (length(GenesOfInterest)>0){
    ...
    text(hit$Nmfc.x, hit$Nmfc.y, labels=hit$GeneSymbol, cex=1, col="black", adj = c(-0.2,0.5), srt=22.5)
  }
  ...
  # Density y axis
  par(mar=c(1,0,0,1))
  par(fig=c(0.75,0.9,0.1,0.75),new=TRUE)
  ...
  # Density x axis
  par(mar=c(0,1,1,0))
  par(fig=c(0.1,0.75,0.75,0.9),new=TRUE)
